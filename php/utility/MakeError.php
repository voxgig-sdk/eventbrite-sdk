...
class EventbriteMakeError
{
    public static function call(?EventbriteContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EventbriteContext([], null);
        }
        $op = $ctx->op ?? new EventbriteOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EventbriteResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EventbriteError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EventbriteError('', $msg, $ctx);
