...
class EventEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = EventbriteSDK::test($seed, null);
        $seen = iterator_to_array($base->Event(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = EventbriteConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = EventbriteSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set EVENTBRITE_TEST_EVENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function event_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("EVENTBRITE_TEST_EVENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["EVENTBRITE_TEST_EVENT_ENTID"]);
    ...
    if ($env["EVENTBRITE_TEST_LIVE"] === "TRUE") {
        ...
        $client = new EventbriteSDK(Helpers::to_map($merged_opts));
    }
