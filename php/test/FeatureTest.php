...
class FtCtrl extends EventbriteControl
{
...
class FtClient
{
    ...
    private EventbriteUtility $utility;

    public function __construct(EventbriteUtility $utility, array $options)
    {
    ...
    public function get_utility(): EventbriteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EventbriteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new EventbriteUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (EventbriteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EventbriteConfig::make_feature($name);
    ...
    private static function buildUrl(EventbriteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EventbriteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EventbriteResponse($fetched);
        }
    ...
    private function fail(EventbriteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EventbriteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
