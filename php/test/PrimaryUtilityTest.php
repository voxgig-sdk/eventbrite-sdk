...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        EventbriteSDK $client,
        EventbriteUtility $utility
    ): EventbriteContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new EventbriteContext($ctxmap, null);
    ...
    private static function fixctx(EventbriteContext $ctx, EventbriteSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?EventbriteError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new EventbriteError($code, $msg);
    }
    ...
    {
        $client = EventbriteSDK::test(null, null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new EventbriteResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new EventbriteBaseFeature();
    ...
    {
        $hook_client = EventbriteSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends EventbriteBaseFeature {
            public $hook_fn = null;
            public function TestHook(EventbriteContext $ctx): void
            {
    ...
    {
        $init_client = EventbriteSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends EventbriteBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new EventbriteSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new EventbriteSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = EventbriteSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = EventbriteSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof EventbriteError) ? $result->err->msg : (string)$result->err;
