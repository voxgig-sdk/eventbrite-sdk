...
class EventEntity
{
    ...
    {
        if ($args) {
            $this->_data = EventbriteHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = EventbriteHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws EventbriteError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EventbriteHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws EventbriteError on failure (item-5 convention).
     */
     ...
     * Create a new Event.
     *
     * @param EventCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EventCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Event|array The created Event as an assoc-array at the
     *   SDK boundary; throws EventbriteError on failure (item-5 convention).
     */
