...
boru-driven command-line client **and** interactive REPL for the Eventbrite
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eventbrite-cli)
make build
...
./eventbrite-cli list event
./eventbrite-cli load 1 event            # {id:1} shorthand
./eventbrite-cli load '{id:1}' event       # explicit match map

# 5. Override the API base URL for a single call
EVENTBRITE_BASE=https://api.example.com ./eventbrite-cli list event

# 6. No arguments -> interactive REPL
./eventbrite-cli
eventbrite> list event
eventbrite> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eventbrite-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/eventbrite-cli list event
   ```
...
```sh
./eventbrite-cli load 1 event          # scalar shorthand for {id:1}
./eventbrite-cli load '{id:1}' event     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `eventbrite>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/eventbrite-cli`. |
...
(`./eventbrite-cli <expr>`) and interactively (the REPL), and expressions compose the
