// Command Eventbrite-cli is a CLI + REPL for the Eventbrite SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EVENTBRITE_APIKEY (api key), EVENTBRITE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EVENTBRITE_APIKEY carries the API key and
	// EVENTBRITE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EVENTBRITE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EVENTBRITE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEventbriteSDK(opts)
