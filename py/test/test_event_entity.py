...
class TestEventEntity:
    ...
    def test_should_stream(self):
        ...
        base = EventbriteSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set EVENTBRITE_TEST_EVENT_ENTID JSON to run live")
        client = setup["client"]
...
def _event_basic_setup(extra):
    ...
    client = EventbriteSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("EVENTBRITE_TEST_EVENT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("EVENTBRITE_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("EVENTBRITE_APIKEY"),
            },
            extra or {},
        ])
        client = EventbriteSDK(helpers.to_map(merged_opts))

    _live = env.get("EVENTBRITE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("EVENTBRITE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
