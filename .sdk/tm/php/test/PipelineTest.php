...
{
    private static function utility(): EventbriteUtility
    {
        return new EventbriteUtility();
    }
    ...
    private static function ctx(array $over = []): EventbriteContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EventbriteContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EventbriteOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EventbriteError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EventbriteOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EventbriteMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EventbriteOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EventbriteMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EventbriteOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EventbriteMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EventbriteSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EventbriteMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EventbriteResponse([]),
            'result' => new EventbriteResult([]),
        ]);
        [, $err] = EventbriteMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EventbriteSpec([]),
            'response' => null,
            'result' => new EventbriteResult([]),
        ]);
        [, $err] = EventbriteMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EventbriteSpec(['step' => 's']),
            'response' => new EventbriteResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EventbriteResult([]),
        ]);
        [, $err] = EventbriteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EventbriteSpec(['step' => 's']),
            'response' => new EventbriteResponse(self::resp(200, ['v' => 1])),
            'result' => new EventbriteResult([]),
        ]);
        [, $err] = EventbriteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EventbriteSpec(['step' => 's']),
            'response' => new EventbriteResponse(self::resp(200, ['v' => 2])),
            'result' => new EventbriteResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EventbriteMakeResponse::call($ctx);
    ...
    {
        $preset = new EventbriteResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EventbriteSpec([]),
            'response' => new EventbriteResponse([]),
            'result' => new EventbriteResult([]),
        ...
        [$got, $err] = EventbriteMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EventbriteResult([])]);
        [, $err] = EventbriteMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EventbriteSpec([]), 'result' => null]);
        [, $err] = EventbriteMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EventbriteSpec(['step' => 's']),
            'result' => new EventbriteResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EventbriteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EventbriteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EventbriteSpec(['step' => 's']),
            'result' => new EventbriteResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EventbriteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EventbriteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EventbriteResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EventbriteSpec([]),
            'result' => new EventbriteResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EventbriteMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EventbriteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EventbriteError('boom', 'boom');
        $utility->fetcher = function (EventbriteContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EventbriteSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EventbriteMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EventbriteContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EventbriteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EventbriteResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EventbriteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EventbriteContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EventbriteSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EventbriteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EventbriteResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EventbriteSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EventbriteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EventbriteMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EventbriteMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EventbriteResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EventbriteDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EventbriteResult(['ok' => false])]);
        $this->expectException(EventbriteError::class);
        EventbriteDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EventbriteResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EventbriteMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EventbriteMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EventbriteBaseFeature();
        $b = new EventbriteBaseFeature();
        EventbriteFeatureAdd::call($ctx, $a);
        EventbriteFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EventbriteBaseFeature
    {
        $f = new EventbriteBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EventbriteFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EventbriteContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EventbriteSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EventbritePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EventbritePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EventbriteResponse(['headers' => null]),
            'result' => new EventbriteResult([]),
        ]);
        EventbriteResultHeaders::call($ctx);
