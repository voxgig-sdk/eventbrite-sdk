...
[MCP](https://modelcontextprotocol.io) server exposing the Eventbrite SDK as
two agent tools — `eventbrite_list` and `eventbrite_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eventbrite-mcp)
make build

# 2. Provide credentials via the environment
export EVENTBRITE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user eventbrite \
  -- /absolute/path/to/eventbrite-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./eventbrite-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// eventbrite_list: first page of records
{ "entity": "event" }
{ "entity": "event", "query": { } }

// eventbrite_load: one record by id
{ "entity": "event", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eventbrite-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user eventbrite \
     -- "$PWD"/dist/*/eventbrite-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `eventbrite_list` and `eventbrite_load` tools now appear
   in new sessions. Ask the agent to *"list event using eventbrite"*
   and it calls `eventbrite_list` with `{"entity":"event"}`.
...
```sh
export EVENTBRITE_APIKEY=sk_live_xxx            # API key
export EVENTBRITE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `eventbrite_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `eventbrite_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EVENTBRITE_APIKEY` | API key sent with every request. |
| `EVENTBRITE_BASE` | Optional override of the API base URL. |
...
```sh
./eventbrite-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"eventbrite_load","arguments":{"entity":"event","query":{"id":1}}}}'
```
