...
function isRunnable(code) {
    return (/new\s+EventbriteSDK\b/.test(code) ||
        /\bEventbriteSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EventbriteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EventbriteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
